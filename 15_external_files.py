...
#		· read([n_chars]) - read at most 'n_chars' from file, or till end if not specified
...
def remove_last_char(text_):  # function to map to each element
	return text_[0:len(text_)-1]  # removes last character
...
print(*map(remove_last_char, file_read_lines.readlines()), sep=' | ')  # '*' expands the map, passing several arguments
